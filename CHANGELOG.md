...
> **本版范围**：PFX 口令**急切迁移** —— 把 v1.6.73 的「下次写入才脱敏」改为「**启动即脱敏**」。
> 基线：`e7d3be6`（= 已发布 v1.6.74）。
>
> ⚠️ 本版改动了二进制 ⇒ **必须重部署**；**不改 Agent**（`cmd/agent` 零改动 ⇒ 无需重新下发）。

### 🐛 修复: 历史明文 PFX 口令在启动时一次性脱敏

- **问题**: v1.6.73 起新写入已脱敏（明文置空 + `pfx_password_enc` + 0600），但**旧 bundle 只在
  「下次被写入」时才转换** ⇒ 长期不续期的 bundle 会无限期保留明文口令（生产实测 **3 个**，
  其中 2 个为**自定义**口令、文件权限 0644）。
- **修复**: 新增 `store.MigratePlaintextPFXPasswords()`，Manager 启动时扫描 `certs/` 下全部
  **在用** bundle：遗留明文 → 密文写入 + 明文置空 + 收敛 0600。
- **安全契约（顺序不可颠倒）**:
  1. 先加密并**回读解密验证**与原文逐字节相等，失败则**不动该文件**（宁留明文，不毁口令）；
  2. 通过后用既有 `atomicWriteFile` **原子替换**（`meta.json` 是 bundle 的真相源，不可半写）；
  3. **幂等**：无明文一律跳过（不改内容 / 权限 / mtime）；
  4. 范围仅限在用 bundle（备份目录不在范围内）；
  5. 失败**不阻断启动**，仅告警。
- **自愈**: 每次启动都执行 ⇒ 幂等，且对「从旧备份恢复」同样自愈。

### 🧪 测试

- `T78a` 转换 + 口令可原样取回 + 0600 + 无临时文件残留 · `T78b` 幂等 · `T78c` 跳过无明文
  （mtime/内容/权限均不动）· `T78d` 容忍缺 meta · `T78e` 全新实例无 `certs/`
- 判别性注入：去掉密文写入（只清明文）⇒ `T78a` **FAIL**（口令被毁）；复原后全过、残留 0

### 📋 涉及文件

`internal/store/pfxmigrate.go`（新增）· `internal/store/pfx_migrate_test.go`（新增）·
`cmd/manager/main.go`（启动接线）— 共 **3 文件**
...
>
> 📦 **本版亦包含 v1.6.73 批次内容**（该批次**未单独发版**，随本版一并发布）：DNS 凭据与证书 PFX 口令的
> **落盘加密**（purpose 隔离派生密钥 · `dns_keys.json` v2 信封 · meta 明文口令置空）· **规格 ⑥ 启动 fail-fast**
> （密钥丢失 ⇒ 拒绝启动，防唯一密文副本被新密钥覆盖）· 受管键口径定型（B-3）· ACME home 告警通道（B-2）·
> 部署前 **preflight 与回退闸门**（Slice 4）· store 级**自死锁守卫**（N-32）· 全仓 gofmt 与 CI 门禁升级（S2/A-7）·
> 发版链修正（N-21 / N-27）。v1.6.73 的详细留痕见仓库内部审计记录。
>
...
> 启发式 ACME home 告警通道（B-2）· 部署前 preflight 与回退闸门（Slice 4）·
> store 级自死锁守卫（N-32）· 全仓 `gofmt` 与 CI 门禁升级（S2/A-7）· 发版链两处小改（N-21/N-27）。
> 基线：`v1.6.72`（`70d5eb0`）。
>
> ⚠️ **本版改动了二进制 ⇒ 必须重部署**（同侧归一 `-buildvcs=false` 实测：基线 `c1e5767253f5743e…`
> → 本版 `f6d7f55f47e2ce12…`；`size` 相同不构成证据）。
> 🔁 **回退前置**：回退到 ≤ 1.6.72 之前**必须先恢复 `dns_keys.json.bak.<TS>` 明文快照** ——
> 旧版读不懂 v2 信封，带着信封回退会把信封当成记录集合解析 ⇒ **DNS 凭据静默丢失**。

### 🔴 安全 1: DNS 凭据此前**明文落盘**（B-1 / F10 / P12）

- **根因**: `data/dns_keys.json` 为 **0600 明文** ⇒ 拿到文件副本即可直接使用云厂商 AK/SK；
  证书 PFX 口令同样以**明文**写在 `certs/<bundle>/meta.json` 的 `pfx_password`。
- **修复**: 新增 purpose 隔离的派生密钥（`dns-keys-at-rest` / `pfx-password-at-rest`，与既有
  `acme-at-rest` **两两不等**，由 T65 断言）：`dns_keys.json` 落盘为 v2 信封
  `{"__ddnskey_enc":2,"ct":"<base64 GCM>"}`；bundle meta 的明文 `pfx_password` **置空**，
  密文改入 `pfx_password_enc` ⇒ **meta.json 全文不含明文口令**。
- **迁移（不就地覆盖）**: 首次读到 v1 明文 ⇒ **先**写 `dns_keys.json.bak.<TS>`（0600，原件字节）
  **再**重写为 v2，并记一条审计；备份写入失败则**放弃迁移**（继续用明文，不阻断服务）。
- **密钥丢失 fail-fast**: `.storage_key` 缺失/不符 ⇒ 启动即拒绝（**不降级空跑**），
  文案提示恢复 `.storage_key` 或从 `.bak`/快照恢复。
- **单一取值入口**: `BundlePFXPassword` / `MetaPFXPassword`（解密优先 ⇒ 明文兼容 ⇒ 默认）；
  **解密失败不静默回落**默认口令（防「静默用错口令」复发）。
- **5 个读取点全部改造**（落盘脱敏后 struct 字段恒为空 ⇒ 任何直读都会退化）：心跳推送 ·
  一致性判定候选 · `UpdateCertMeta` 双 PFX 重建 · 证书 detail 接口 · 重建写回点。
  其中「重建」若退化为默认口令会**覆盖用户自定义口令**（数据丢失）⇒ 解析失败时**拒绝重建**。

### 🔴 修复 2: 心跳推送会把自定义 PFX 口令**强制回默认**（B-1 Slice 3b）

- **根因**: 旧逻辑为「盘载 `PFXPassword == ""` ⇒ 回填默认口令并落盘」，而落盘脱敏后该判据
  **恒真** ⇒ 每次心跳都把自定义口令改回默认，Agent 会以错口令导入 PFX。
- **修复**: 改经单一取值入口取值；**解析失败则跳过本次下发**（绝不下发错口令），并记 error 审计。

### 🟠 修复 3: 自动续签可能**覆盖用户自定义 PFX 口令**

- `UpdateCertMeta` 原直读 meta 的 `pfx_password`（落盘脱敏后恒空）⇒ 会以**默认口令**重建
  双 PFX 并下发；现经解析器取解密值，**解析失败时跳过重建并记日志**（保留既有 PFX）。

### 🟠 修复 4: 旧 meta 的受管键被覆盖时**静默**（B-3）

- 为 `CertBundle` 的**受管键集合**给出裁决依据（由 struct 字段驱动、必须权威落盘的键）；
  把「**struct 先合并、extra 后合并**」定为**顺序不变量**（改动该顺序等价撤销「`meta.domains`
  被置空」的修复）；旧 meta 的受管键与 struct 不一致时记 **1 条 warning 审计**（仅键名、不含值）；
  未登记键（含 `x-` 前缀）一律**原样保留**。

### 🟡 修复 5: 启发式 ACME home 的位置提示**参与判定**（B-2）

- 位置/体检类提示改走 **Caveats** 通道（审计类别 `home 告警`，warning，跨 flush 去重），
  **不参与** adopt/skip 判定 ⇒ 不再因「祖先目录组可写」把启发式解析出的 home 误判跳过。

### 🟢 新增运维闸门

- **部署前 preflight**（任一失败即**拒绝部署**）：`.storage_key` 存在/非空/0600（**必须早于
  首次 `NewStore`** —— 否则会被自动补生成，判据恒真）、canonical 令牌**真解一次**、对**现存密文**
  试解（`dns_keys.json` + ACME 账号面）、落盘形状判定与迁移路径预演、备份就位、
  `.storage_key` 可恢复性（与备份逐字节一致）。
- **回退闸门**：`dns_keys.json` 必须①已由 `.bak.<TS>` 恢复（逐字节一致）②解出键集非空
  ③**不含哨兵键** —— 否则**拒绝回退**。
- **store 级自死锁守卫**：运行时看门狗（公开入口含回调路径）+ 结构性断言（取锁函数不得抵达
  取锁函数；`*Locked` 助手不得取锁）+ 回调调用点白名单（持锁回调必须保留契约注释）。

### 🧹 卫生（S2 / A-7）

- 全仓 `gofmt` 清理 **25** 文件（排除 `cmd/_deprecated` 4 个，并在 CI 显式排除 + 独立断言）；
  CI 门禁由「仅检查改动文件」升级为**全仓版**（显式枚举 + 数量断言 + 排除项独立断言 + 0 值守卫）。
  注：**`gofmt` 也会改二进制**（行号 → pclntab ⇒ 哈希变）⇒ 已更正此前「gofmt 不改二进制」的口径。

### 🔧 发版链（N-21 / N-27）

- `release.yml` 显式记录 **CI 工具链**（`go1.25`；生产部署件为 `go1.26.4` ⇒ 二者产物
  **不可逐字节比较**；统一版本列为独立项）。
- Release 正文的 SHA256 块**纳入 Manager**（此前只有 3 个 Agent ⇒ Manager 的 `.sha256`
  仅作资产出现）。

### 📌 升级注意

1. **必须重部署**（本版动二进制）；备份**必须包含 `.storage_key`** —— 否则数据面恢复后密文不可解。
2. 首次启动会迁移 `dns_keys.json`（写 `.bak.<TS>` 并记审计）；`.bak` **保留期 ≥ 一个观察窗口**
   （首个 tick + 24h），由部署执行人在观察窗口通过后 `shred -u` 显式清理并留痕。
3. **UI 契约不变**：证书详情仍返回**明文** PFX 口令供查看/复制。
