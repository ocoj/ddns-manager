...
| 底部 | 版本号 `v1.5.9`，11px 灰色，页面加载时通过 `/api/ping` 动态拉取 |
...
│                                                                │
│  ┌─ 卡片 1 ── [DNS配置 1          ] [删除] ─────────────────┐  │
│  │  DNS Key: [阿里-蓝迅 (LTAI5t...) ▼]  TTL: [600s ▼]        │  │
│  │                                                            │  │
│  │  ■ IPv4                                         [☐ 启用]  │  │
│  │    获取方式: [URL ▼]  URL: [(默认)                     ]   │  │
│  │    域名: [win10.example.com] [✕]  [+ 添加]                 │  │
│  │                                                            │  │
│  │  ■ IPv6                                         [☑ 启用]  │  │
│  │    获取方式: [网卡 ▼]  网卡: [Ethernet0 (192.168.1/240e:) ▼]│  │
│  │    域名: [win10.example.com] [✕]  [+ 添加]                 │  │
│  └────────────────────────────────────────────────────────────┘  │
│                                                                │
│  ┌─ 卡片 2 ── [DNS配置 2          ] [删除] ─────────────────┐  │
│  │  DNS Key: [腾讯云-诺轩2 ▼]  TTL: [600s ▼]                 │  │
│  │  IPv4 未启用 / IPv6 网卡方式 → win10.example.org              │  │
│  └────────────────────────────────────────────────────────────┘  │
│                                                                │
│  [+ 新增 DNS 配置]                                              │
│                                                                │
│  ══ 证书部署（节点统一） ═══════════════════════════════════  │
│  证书: [acme-*.example.org ▼]  部署路径: [C:\...\acme-_.example.org] [✕] │
│  [+ 添加证书绑定]                                              │
│                                                                │
│  [❓ 帮助]               [取消]  [保存并下发]                    │
└────────────────────────────────────────────────────────────────┘
...
### 规则
1. 多张 DNS 配置卡片独立管理，每张可绑定不同的 DNS Key/IPv4/IPv6/域名/TTL
2. 获取方式切换 → 对应输入区动态替换，网卡下拉显示 `网卡名 (IPv4 / IPv6)`
3. 网卡列表从 `hardware.interfaces` 心跳数据加载，含 IPv4/IPv6 详情
4. URL 留空 → 服务端自动填入默认值（`renderDDNSConfig` 兜底）
5. 域名和证书绑定支持动态增减
6. **防呆**: 启用 IPv4/IPv6 但无域名 → 自动禁用；保存核验 DNS Key/网卡/命令必填
7. **帮助按钮**: 底部 ❓ 弹窗含 URL/命令/网卡获取方式说明与 ddns-go 社区示例
8. 保存调用 `PUT /api/admin/nodes/{id}/config`
  ...
  "dns_confs": [
    {
      "name": "DNS配置 1",
      "dns_key": "阿里-蓝迅",
      "ipv4": {"enable": false, "gettype": "url", "domains": []},
      "ipv6": {"enable": true, "gettype": "netInterface", "netinterface": "Ethernet0 2",
               "domains": [{"domain": "win10.example.com"}]},
      "ttl": "600"
    },
    {
      "name": "DNS配置 2",
      "dns_key": "腾讯云-诺轩2",
      "ipv6": {"enable": true, "gettype": "netInterface",
               "domains": [{"domain": "win10.example.org"}]},
      "ttl": "600"
    }
  ],
  "cert_bindings": [
    {"bundle_name": "acme-*.example.org", "deploy_path": "C:\ddns-agent\certs\acme-_.example.org"}
  ]
}
```

> 旧格式 `ipv4`/`ipv6`/`dns_key_name` 字段保留向后兼容，新配置优先使用 `dns_confs`。
...
| `GET` | `/api/admin/logs` | 查询日志，参数: `?category=&status=&node=&from=&to=&limit=100&offset=0`。返回: `{"events","total","limit","offset","categories"}`。`total` 为过滤后匹配总数（v1.5.11修复），`limit`最大500 |
...
- 移动端响应式适配
- 移动端表格 → 卡片布局: `desk-only` / `mob-only` CSS 双渲染, JS 同时生成 `<table>` 和 `<div>` 卡片两套 HTML, `@media(max-width:768px)` 切换显示
  - DNS Keys / 证书 / ACME / 已上传二进制 / 节点版本 全部采用此模式
  - 卡片使用 flexbox 精确分行, 避免 CSS 强行改造 `<table>` 的对齐难题
